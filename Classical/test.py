...
def experiment(n: int, tau: float, theta: float, error_rate: float, runs: int = 1000) -> float:
    ...
    for _ in range(runs):
        ...
        predictions = generate_predicted_values_uniform(real_values, error_rate)
...
if __name__ == "__main__":
    ...
    for theta in thetas:
        ...
        fig.add_trace(go.Scatter(
            ...
            y=y_success_rates,
            mode='lines+markers',
            name=f'Success Rate (θ={theta})',
            hovertemplate=f'<b>θ={theta}</b><br>Error: %{{x}}<br>Rate: %{{y:.3f}}<extra></extra>'
        ))

    # Ρυθμίσεις εμφάνισης 
    fig.update_layout(
        title=f"Tο Πρόβλημα του Γραμματέα (n={n}, τ={tau}) (Success Rates). Διάφορες τιμές του θ.",
